...
from aula.apps.relacioFamilies.forms import (
    AlumneModelForm,
    ResponsableModelForm,
    comunicatForm,
    escollirAlumneForm,
)
...
from aula.apps.usuaris.tools import (
    bloqueja,
    creaNotifUsuari,
    desbloqueja,
    enviaBenvingudaAlumne,
    getRol,
    testEmail,
)
...
def enviaBenvinguda(request, pk):
    ...
    return render(
        ...
        {
            "msgs": cosMissatge,
            "head": "Acció Envia Benviguda a {0}".format(alumne),
        },
    )
...
def bloquejaDesbloqueja(request, pk):
    ...
    if actiu:
        resultat = bloqueja(
            alumne, "Bloquejat per {0} amb data {1}".format(professor, datetime.now())
        )
    else:
...
def configuraConnexio(request, pk):
    ...
    if alumne.dadesaddicionalsalumne_set.exists():
        for element in CUSTOM_DADES_ADDICIONALS_ALUMNE:
            if "Tutor" in element["visibilitat"]:
                try:
                    valor = alumne.dadesaddicionalsalumne_set.get(
                        label=element["label"]
                    ).value
                except:  # noqa: E722
                    valor = "-"
                infoForm.append((element["label"] + "(Esfer@/Saga)", valor))
    ...
    ResponsableFormSet = modelform_factory(Responsable, form=ResponsableModelForm)
    ...
    if request.method == "POST":
        ...
        if resp1:
            formResp1 = ResponsableFormSet(
                "resp1", request.POST, request.FILES, instance=resp1
            )
        if resp2:
            formResp2 = ResponsableFormSet(
                "resp2", request.POST, request.FILES, instance=resp2
            )
        hihaerrors = False
        if (
            formAlmn.is_valid()
            and (not resp1 or formResp1.is_valid())
            and (not resp2 or formResp2.is_valid())
        ):
            ...
            if res < -1:
                errors.setdefault("correu", []).append("""Adreça no vàlida""")
            if len(errors) > 0:
            ...
            if resp1:
                ...
                res, email = testEmail(email, False)
                if res < -1:
                    errors.setdefault("correu_relacio_familia", []).append(
                        """Adreça no vàlida"""
                    )
                if len(errors) > 0:
        ...
        if resp1:
            formResp1 = ResponsableFormSet("resp1", instance=resp1)
        if resp2:
            formResp2 = ResponsableFormSet("resp2", instance=resp2)

    formset = []
    formAlmn.infoForm = infoForm
    formset.append(formAlmn)
    if resp1:
        formset.append(formResp1)
    if resp2:
        formset.append(formResp2)
...
def dadesRelacioFamilies(request):
    ...
    for grup in grups:
        ...
        capcelera.contingut = "Correus Contacte"
        taula.capceleres.append(capcelera)

        capcelera = tools.classebuida()
        capcelera.amplade = 25
        capcelera.contingut = "Estat"
        taula.capceleres.append(capcelera)
        ...
        familia_pendent_de_mirar_models = [
            ...
            (
                "activitats o pagaments",
                NotificaSortida,
            ),
            (
                "incidencies o observacions",
                Incidencia,
            ),
            (
                "sanció(ns)",
        ...
        for alumne in alumnes:
            ...
            if nConnexions > 0:
                dataDarreraConnexio.append(
                    alumne.user_associat.LoginUsuari.filter(exitos=True)
                    .order_by("-moment")[0]
                    .moment
                )
            for r in alumne.get_responsables():
                if not r:
                    continue
                if not bool(r.motiu_bloqueig):
                    bloquejat_text = []
                if r.get_user_associat():
                    count = (
                        r.get_user_associat().LoginUsuari.filter(exitos=True).count()
                    )
                else:
                    count = 0
                ...
                if count > 0:
                    dataDarreraConnexio.append(
                        r.get_user_associat()
                        .LoginUsuari.filter(exitos=True)
                        .order_by("-moment")[0]
                        .moment
                    )
            camp.multipleContingut = bloquejat_text + [
                (
                    "( {0} connexs. )".format(nConnexions),
                    None,
                ),
            ]
            if nConnexions > 0:
                camp.multipleContingut.append(
                    (
                        "Darrera Connx: {0}".format(
                            max(dataDarreraConnexio).strftime("%d/%m/%Y")
                        ),
                        None,
                    )
                )
            ...
            camp.enllac = None
            accio_list = [
                ("Configura", "/open/configuraConnexio/{0}".format(alumne.pk)),
                # (u'Bloquejar' if alumne.esta_relacio_familia_actiu() else u'Desbloquejar', '/open/bloquejaDesbloqueja/{0}'.format(alumne.pk)),
                ("Envia benvinguda", "/open/enviaBenvinguda/{0}".format(alumne.pk)),
                ("Veure Portal", "/open/elMeuInforme/{0}".format(alumne.pk)),
            ]
...
def canviParametres(request):
    ...
    infoForm = [
        ...
        (
            ...
            str(edatAlumne)
            + " ("
            + formats.date_format(alumne.data_neixement, "SHORT_DATE_FORMAT")
            + ")",
        ),
        ("RALC", alumne.ralc),
        ("Responsable", dades1),
    ...
    if dades2:
        infoForm.append(("Responsable", dades2))
    ...
    if request.method == "POST":
        ...
        if responsable:
            formResp = ResponsableFormSet(
                "resp", request.POST, request.FILES, instance=responsable
            )
        hihaerrors = False
        ...
        if responsable:
            formResp = ResponsableFormSet("resp", instance=responsable)

    formset = []
    formAlmn.infoForm = infoForm
    formset.append(formAlmn)
    if responsable:
        formset.append(formResp)
...
def choices_hores(alumne, dia, actual=True):
    if not bool(alumne):
        ...
        hores = get_hores(alumne, dia, actual)
...
def horesAlumneAjax(request, idalumne, dia):
    ...
    if request.method == "GET":  # request.is_ajax()
        ...
        message = '<option value="0" selected>- dia complet -</option>'
        for h in hores:
            message += '<option value="%s">%s</option>' % (h[0], h[1])
...
def comunicatAbsencia(request):
    ...
    if not alumne or (not responsable and alumne.edat() < 18):
        messages.info(
            request,
            "Els comunicats d'alumnes menors d'edat els ha de fer un responsable.",
        )
        return HttpResponseRedirect("/")
    ...
    if not primerdia:
        messages.info(
            request, "No és possible fer comunicats. No té programada cap classe."
        )
        return HttpResponseRedirect("/")
    ...
    if primerdia > (ara + timedelta(days=diesantelacio)).date():
        messages.info(
            request,
            "Només es poden fer comunicats amb antelació màxima d'una setmana. La següent classe serà el dia {0}.".format(
                primerdia.strftime("%d/%m")
            ),
        )
        return HttpResponseRedirect("/")
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if int(horai) == 0:
                ...
                horai = (
                    hores[0][0]
                    if hores and hores[0]
                    else FranjaHoraria.objects.first().id
                )
            if int(horaf) == 0:
                hores = get_hores(alumne, dataf)
                horaf = (
                    hores[-1][0]
                    if hores and hores[-1]
                    else FranjaHoraria.objects.last().id
                )
            horai = FranjaHoraria.objects.get(id=horai).hora_inici
            horaf = FranjaHoraria.objects.get(id=horaf).hora_fi
            enviaMsg(
                alumne.get_user_associat(),
                credentials,
                alumne,
                datai,
                horai,
            ...
            messages.info(
                request,
                "Comunicat d'absència per {}, enviat correctament".format(
                    unicode(alumne.nom)
                ),
            )
            return HttpResponseRedirect(url_next)
...
def comunicatsAnteriors(request):
    ...
    RequestConfig(
        request, paginate={"paginator_class": DiggPaginator, "per_page": 25}
    ).configure(table)
...
def elMeuInforme(request, pk=None):
    ...
    if detall in ["all", "assistencia"]:
        controls = alumne.controlassistencia_set.exclude(
            estat__codi_estat__in=["P", "O"]
        ).filter(estat__isnull=False)
        controlsNous, creaNotif = getNousElements(controls, user)
        if creaNotif and not notifAlumne:
            notifAlumne = creaNotifUsuari(user, alumne, "N")
        if controlsNous and not revisAlumne:
        ...
        capcelera = tools.classebuida()
        capcelera.amplade = 25
        capcelera.contingut = "Dia"
        capcelera.enllac = ""
        taula.capceleres.append(capcelera)

        capcelera = tools.classebuida()
        capcelera.amplade = 40
        capcelera.contingut = "Falta, assignatura i franja horària."
        taula.capceleres.append(capcelera)

        capcelera = tools.classebuida()
        capcelera.amplade = 20
        capcelera.contingut = "Notificada"
        taula.capceleres.append(capcelera)

        capcelera = tools.classebuida()
        capcelera.amplade = 20
        capcelera.contingut = "Revisada"
        taula.capceleres.append(capcelera)
        ...
        for control in tots_els_controls:
            ...
            camp = tools.classebuida()
            camp.enllac = None
            camp.contingut = "{0}".format(notificacio)
            camp.negreta = negreta
            filera.append(camp)

            camp = tools.classebuida()
            camp.enllac = None
            camp.contingut = "{0}".format(revisio)
            camp.negreta = negreta
            filera.append(camp)
            ...
            if not semiImpersonat:
                if not notificacio:
                    control.set_notificacio(notifAlumne)
                if not revisio:
                    control.set_revisio(revisAlumne)
        ...
        if creaNotif and not notifAlumne:
            notifAlumne = creaNotifUsuari(user, alumne, "N")
        if observacionsNoves and not revisAlumne:
            revisAlumne = creaNotifUsuari(user, alumne, "R")
        ...
        capcelera.contingut = "Professor i observació."
        taula.capceleres.append(capcelera)
        ...
        for incidencia in observacions.order_by(
            "-dia_incidencia", "-franja_incidencia"
        ):
            ...
            if not semiImpersonat:
                if not notificacio:
                    incidencia.set_notificacio(notifAlumne)
                if not revisio:
                    incidencia.set_revisio(revisAlumne)
        ...
        if creaNotif and not notifAlumne:
            notifAlumne = creaNotifUsuari(user, alumne, "N")
        if incidenciesNoves and not revisAlumne:
            revisAlumne = creaNotifUsuari(user, alumne, "R")
        ...
        capcelera = tools.classebuida()
        capcelera.amplade = 15
        capcelera.contingut = "Dia i estat"
        capcelera.enllac = ""
        taula.capceleres.append(capcelera)

        capcelera = tools.classebuida()
        capcelera.amplade = 50
        capcelera.contingut = "Professor i Incidència"
        taula.capceleres.append(capcelera)
        ...
        if creaNotif and not notifAlumne:
            notifAlumne = creaNotifUsuari(user, alumne, "N")
        if expulsionsNoves and not revisAlumne:
            revisAlumne = creaNotifUsuari(user, alumne, "R")
        ...
        capcelera = tools.classebuida()
        capcelera.amplade = 10
        capcelera.contingut = "Data comunicació"
        taula.capceleres.append(capcelera)

        capcelera = tools.classebuida()
        capcelera.amplade = 60
        capcelera.contingut = "Professor i motiu"
        taula.capceleres.append(capcelera)
        ...
        for expulsio in expulsions.order_by("-dia_expulsio", "-franja_expulsio"):
            ...
            camp.contingut = "Sr(a): {0} - {1}".format(
                expulsio.professor,
                expulsio.motiu if expulsio.motiu else "Pendent redactar motiu.",
            )
            camp.negreta = negreta
            filera.append(camp)
            ...
            if not semiImpersonat:
                if not notificacio:
                    expulsio.set_notificacio(notifAlumne)
                if not revisio:
                    expulsio.set_revisio(revisAlumne)
    ...
    if detall in ["all", "incidencies"]:
        ...
        if creaNotif and not notifAlumne:
            notifAlumne = creaNotifUsuari(user, alumne, "N")
        if sancionsNoves and not revisAlumne:
            revisAlumne = creaNotifUsuari(user, alumne, "R")
        ...
        capcelera = tools.classebuida()
        capcelera.amplade = 15
        capcelera.contingut = "Dates"
        capcelera.enllac = ""
        taula.capceleres.append(capcelera)

        capcelera = tools.classebuida()
        capcelera.amplade = 60
        capcelera.contingut = "Detall"
        taula.capceleres.append(capcelera)
        ...
        for sancio in sancions.order_by("-data_inici"):
            ...
            if not semiImpersonat:
                if not notificacio:
                    sancio.set_notificacio(notifAlumne)
                if not revisio:
                    sancio.set_revisio(revisAlumne)
    ...
    if detall in ["all", "dades"]:
        ...
        camp.enllac = None
        camp.contingut = "{0}".format(
            str(alumne) if responsable else alumne.get_nom_sentit()
        )
        filera.append(camp)
        ...
        camp = tools.classebuida()
        camp.enllac = None
        camp.contingut = "{0} Tutor(a): {1}".format(
            alumne.grup, ", ".join([str(t) for t in alumne.tutorsDelGrupDeLAlumne()])
        )
        filera.append(camp)
        ...
        camp.contingut = "{0} - ({1} {2})".format(
            alumne.adreca, alumne.cp, localitat_i_o_municipi
        )
        filera.append(camp)
        ...
        if CUSTOM_DADES_ADDICIONALS_ALUMNE:
            ...
            for dada_addicional in dades_addicionals:
                ...
                if (
                    element
                    and dada_addicional.label in labels
                    and "Familia" in element["visibilitat"]
                ):
                    filera = []
                    camp = tools.classebuida()
                    camp.enllac = None
    ...
    infSortida = (
        detall in ["all", "sortides"]
        and settings.CUSTOM_MODUL_SORTIDES_ACTIU
        and not settings.CUSTOM_SORTIDES_OCULTES_A_FAMILIES
    )
    pagquotes = QuotaPagament.objects.filter(alumne=alumne, quota__importQuota__gt=0)
    pagquotesNoves, creaNotif = getNousElements(
        pagquotes.filter(pagament_realitzat=False), user
    )
    if creaNotif and not notifAlumne:
        notifAlumne = creaNotifUsuari(user, alumne, "N")
    if pagquotesNoves and not revisAlumne:
        revisAlumne = creaNotifUsuari(user, alumne, "R")

    infQuota = detall in ["all", "sortides"] and (
        pagquotes or settings.CUSTOM_QUOTES_ACTIVES
    )
    ...
    if infSortida:
        ...
        sortidespagadesnonotificades = Sortida.objects.filter(
        ...
        if creaNotif and not notifAlumne:
            notifAlumne = creaNotifUsuari(user, alumne, "N")
        if sortidesNoves and not revisAlumne:
            revisAlumne = creaNotifUsuari(user, alumne, "R")
        ...
        taula.codi = nTaula
        nTaula += 1
        taula.tabTitle = "{0} {1}".format(
            titol_sortides, pintaNoves(sortidesNoves.count() + pagquotesNoves.count())
        )
        ...
        for act in activitats.order_by("-calendari_desde"):
            ...
            if act.pk in sortides_on_no_assistira:
                comentari_no_ve = "NO INSCRIT A L’ACTIVITAT."
                if act.pk in sortides_justificades:
                    comentari_no_ve += (
                        "NO INSCRIT A L’ACTIVITAT. Té justificada l'absència."
                    )
            ...
            camp = tools.classebuida()
            camp.enllac = None
            camp.contingut = "{0}".format(act.titol)
            camp.negreta = negreta
            filera.append(camp)
            # ----------------------------------------------
            camp = tools.classebuida()
            ...
            camp.modal["txtboto"] = "Detalls"
            camp.modal["tittle"] = "{0} ({1})".format(
                act.titol,
            ...
            if act.tipus == "P":
                camp.modal["body"] = "{0}\n{1}\n{2}\n{3}\n{4}".format(
                    ...
                    (
                        "Preu: {0} €".format(str(act.preu_per_alumne))
                        if act.preu_per_alumne
                        else "Preu: 0 €"
                    ),
                    (
                        "Data límit pagament: {0}".format(str(act.termini_pagament))
                        if act.termini_pagament
                        else ""
                    ),
                )
            else:
            ...
            if act.tipus_de_pagament == "ON":
                ...
                alumne_tret_de_lactivitat = (
                    alumne in act.alumnes_que_no_vindran.all()
                    or alumne in act.alumnes_justificacio.all()
                )
                camp = tools.classebuida()
                ...
                if not alumne_tret_de_lactivitat:
                    pagament_sortida_alumne = get_object_or_404(
                        SortidaPagament, alumne=alumne, sortida=act
                    )
                    camp.id = pagament_sortida_alumne.id
                    # Pagaments pendents o ja fets. Si sortida caducada no mostra pagament pendent.
                    if (
                        (
                            act.termini_pagament
                            and act.termini_pagament >= datetime.now()
                        )
                        or not bool(act.termini_pagament)
                        or pagament_sortida_alumne.pagamentFet
                    ):
                        if pagament_sortida_alumne.pagamentFet:
                            ...
                            if settings.CUSTOM_SORTIDES_PAGAMENT_ONLINE:
                                camp.buto = "sortides__sortides__pago_on_line"
            ...
            taula.fileres.append(filera)
            if not semiImpersonat:
                if not notificacio and notifica:
                    notifica.first().set_notificacio(notifAlumne)
                if not revisio and notifica:
                    notifica.first().set_revisio(revisAlumne)
    ...
    if infQuota:
        if not infSortida:
            ...
            taula.tabTitle = "{0} {1}".format(
                titol_sortides, pintaNoves(pagquotesNoves.count())
            )
        ...
        for pagquota in pagquotes.order_by(
            "-quota__any", "-quota__dataLimit", "dataLimit"
        ):
            ...
            camp = tools.classebuida()
            camp.enllac = None
            camp.contingut = "{0}".format(pagquota.quota.descripcio)
            camp.negreta = negreta
            filera.append(camp)
            # ----------------------------------------------
            camp = tools.classebuida()
            ...
            camp.modal["body"] = "{0}\n{1}\n{2}".format(
                (
                    ...
                    if settings.CUSTOM_SORTIDES_PAGAMENT_ONLINE
                        ...
                        if settings.CUSTOM_SORTIDES_PAGAMENT_CAIXER
                        else settings.CUSTOM_SORTIDES_INSTRUCCIONS_PAGAMENT
                    )
                ),
                "Preu: {0} €".format(valor),
                (
                    "Data límit pagament: {0}".format(str(pagquota.getdataLimit))
                    if pagquota.getdataLimit
                    else ""
                ),
            )
            filera.append(camp)
            ...
            taula.fileres.append(filera)
            if not semiImpersonat:
                if not notificacio and not revisio:
                    pagquota.set_notificacio(notifAlumne)
                if not revisio:
                    pagquota.set_revisio(revisAlumne)

        report.append(taula)

    # ----Qualitativa -----------------------------------------------------------------------------
    qualitatives_alumne = {
        r.qualitativa for r in alumne.respostaavaluacioqualitativa_set.all()
    }
    avui = datetime.now().date()
    qualitatives_en_curs = [
        q
        for q in qualitatives_alumne
        if (
    ...
    if detall in ["all", "qualitativa"] and qualitatives_en_curs:
        ...
        if creaNotif and not notifAlumne:
            notifAlumne = creaNotifUsuari(user, alumne, "N")
        if respostesNoves and not revisAlumne:
            revisAlumne = creaNotifUsuari(user, alumne, "R")
        ...
        asignatura_label = (
            assignatures[0].tipus_assignatura.capcelera
            if hi_ha_tipus_assignatura
            else "Matèria"
        )
        ...
        taula.codi = nTaula
        nTaula += 1
        taula.tabTitle = "Avaluació qualitativa {0}".format(
            "!" if respostesNoves.exists() else ""
        )
        ...
        def keyf(x):
            return x.qualitativa.nom_avaluacio + x.assignatura.nom_assignatura
        ...
        for _, respostes in groupby(respostes_sort, keyf):
            ...
            camp.enllac = None
            camp.contingut = (
                respostes[0].assignatura.nom_assignatura or respostes[0].assignatura
            )
            camp.negreta = negreta
            filera.append(camp)
            ...
            for resposta in respostes:
                camp.multipleContingut.append(
                    (
                        "{0}".format(resposta.get_resposta_display()),
                        None,
                    )
                )
            camp.negreta = negreta
            ...
            if not semiImpersonat:
                for resposta in respostes:
                    ...
                    if not notificacio:
                        resposta.set_notificacio(notifAlumne)
                    if not revisio:
                        resposta.set_revisio(revisAlumne)
...
def pintaNoves(numero):
    txt = ""
    if numero == 1:
        txt = "({0} nova)".format(numero)
    elif numero > 1:
        txt = "({0} noves)".format(numero)
    return txt
