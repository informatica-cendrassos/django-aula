...
from aula.apps.presencia.business_rules.impartir import (
    impartir_despres_de_passar_llista,
)
from aula.apps.presencia.forms import (
    ControlAssistenciaForm,
    ...
    faltesAssistenciaEntreDatesForm,
    llistaLesMevesHoresForm,
    marcarComHoraSenseAlumnesForm,
    passaLlistaGrupDataForm,
    regeneraImpartirForm,
)
...
from aula.apps.presencia.rpt_faltesAssistenciaEntreDatesProfessor import (
    faltesAssistenciaEntreDatesProfessorRpt,
)
from aula.apps.presenciaSetmanal.views import ProfeNoPot
from aula.apps.sortides.models import Sortida
from aula.apps.usuaris.models import Accio, User2Professor
...
def regeneraImpartir(request):
    head = "Reprogramar classes segons horari actual"
...
def mostraImpartir(request, year=None, month=None, day=None):
    ...
    impartir_pendents = (
        []
    )  # aquí les que potser no posarem (si estan buides i no hi ha més)
    dies_calendari = []
    ...
    for f in FranjaHoraria.objects.all():
        ...
        for d in range(0, 5):
            dia = data + d * unDia
            if dia not in dies_calendari:
                dies_calendari.append(dia)
            ...
            imparticions = [
                (
                    ...
                    getSoftColor(
                        str(x.horari.grup)
                        + (
                            x.horari.assignatura.codi_assignatura
                            if x.horari.assignatura
                            else ""
                        )
                    ),  #
                    x.color(),
                ...
                for x in Impartir.objects.filter(
                    franja_impartir & dia_impartir & (user_impartir | guardia)
                )
            ]

            impartir_franja.append((imparticions, dia == data_actual))
            te_imparticions = (
                te_imparticions or imparticions
            )  # miro si el professor ha d'impartir classe en aquesta franja

        if te_imparticions:  # si ha d'impartir llavors afageixo la franja
            primera_franja_insertada = True
            impartir_tot += (
                impartir_pendents  # afegeixo franges buides entre franges "plenes"
            )
            impartir_pendents = []
            impartir_tot.append(impartir_franja)
        else:
            if primera_franja_insertada:
                impartir_pendents.append(
                    impartir_franja
                )  # franges buides que potser caldrà afegir a l'horari
    ...
    msg = "Has controlat presència en un {0}% de les teves classes".format(pct)
    ...
    calendari = [
        (_date(d, "D"), d.strftime("%d/%m/%Y"), d == data_actual)
        for d in dies_calendari
    ]
    ...
    q_sortida_no_inclou_setmana_vinent = (
        q_fi_sortida_menor_que_dilluns | q_inici_sortida_despres_divendres
    )
    # q es meu
    q_es_meu = Q(horari__professor=professor)

    imparticions_daquests_dies = Impartir.objects.filter(
        ~q_sortida_no_inclou_setmana_vinent & q_es_meu
    )

    alumnes_surten = any(
        x.hi_ha_alumnes_amb_activitat_programada for x in imparticions_daquests_dies
    )

    if alumnes_surten:
        msg = """Properament hi ha activitats previstes on els teus alumnes hi participen."""
...
def passaLlista(request, pk):
    ...
    ultimaReserva = (
        ReservaAula.objects.filter(dia_reserva=impartir.dia_impartir)
    ...
    if ultimaReserva is not None:
        ...
        if esUltimaHora:
            msg = "ATENCIÓ: és l'última hora en aquesta aula. Recorda't de pujar les cadires, tancar finestres, baixar persianes i deixar l'aula ordenada."
            ...
            *settings.CUSTOM_ORDER_PRESENCIA
            ...
            if control_a.nohadeseralaula_set.exists():
                ...
                try:
                    ...
                    hiHaRetard |= bool(control_aux.estat.codi_estat) and (
                        control_aux.estat.codi_estat == "R"
                    )
                    quelcomBe |= True
                except (ProfeNoPot, ValidationError) as e:
                    totBe = False
                    # Com que no és un formulari de model cal tractar a mà les incidències del save:
                    form = helper_tuneja_item_nohadeseralaula(
                        request, control_a, te_error=True
                    )
                    if form._errors is None:
                        form._errors = (
                            ErrorDict()
                        )  # en alguns casos arriba sense _errors IDNW
        ...
        if quelcomBe:
            ...
            try:
                ...
                if hiHaRetard and settings.CUSTOM_RETARD_PROVOCA_INCIDENCIA:
                    url_incidencies = reverse(
                        "aula__horari__posa_incidencia", kwargs={"pk": pk}
                    )
                    msg = """Has posat 'Retard', recorda que els retards provoquen incidències, 
                ...
                Accio.objects.create(
                    ...
                    text="""Passar llista: {0}.""".format(impartir),
                )
    ...
    for form in formset:
        if hasattr(form, "instance"):
            ...
            form.hora_anterior = (
                0
                if d["assistenciaaHoraAnterior"] == "Present"
                else (
                    1
                    if d["assistenciaaHoraAnterior"] == "Absent"
                    else 2 if d["assistenciaaHoraAnterior"] == "Online" else None
                )
            )
            prediccio, pct = predictTreeModel(d)
            form.prediccio = (
                0 if prediccio == "Present" else 1 if prediccio == "Absent" else None
            )

            form.avis = None
            form.avis_pct = ("{0:.2f}%".format(pct * 100)) if pct else ""
            if pct < 0.8:
                form.bcolor = "#CC0000"
                form.avis = "danger"
            elif pct < 0.9:
    ...
    def el_puc_justificar(i):
        return (
            not settings.CUSTOM_NOMES_TUTOR_POT_JUSTIFICAR
            or User2Professor(user) in i.alumne.tutorsDeLAlumne()
        )

    els_meus_tutorats = ",".join(
        unicode(i.pk)
...
def helper_tuneja_item_nohadeseralaula(request, control_a, te_error=False):
    ...
    q_no_al_centre_expulsat = control_a.nohadeseralaula_set.filter(
        motiu=NoHaDeSerALAula.EXPULSAT_DEL_CENTRE
    )
    q_no_al_centre_sortida = control_a.nohadeseralaula_set.filter(
        motiu=NoHaDeSerALAula.SORTIDA
    )
    q_no_al_centre_altres = control_a.nohadeseralaula_set.exclude(
        motiu__in=[NoHaDeSerALAula.EXPULSAT_DEL_CENTRE, NoHaDeSerALAula.SORTIDA]
    )
    if q_no_al_centre_expulsat.exists():
        form = ControlAssistenciaFormFake()
        form.fields["estat"].label_suffix = ""
        form.fields["estat"].label = unicode(alumne) + ", ".join(
            [
                ": sanció del {0} al {1}".format(
                    x.sancio.data_inici.strftime("%d/%m/%Y"),
        ...
        form = ControlAssistenciaFormFake()
        form.fields["estat"].label_suffix = ""
        form.fields["estat"].label = (
            unicode(alumne)
            + " - Activitat: "
            + ", ".join([x.sortida.titol for x in q_no_al_centre_sortida.all()])
        )
        ...
        form.fields["estat"].label_suffix = ""
        form.fields["estat"].label = unicode(alumne) + " - ".join(
            [x.get_motiu_display() for x in q_no_al_centre_altres.all()]
        )

    else:
        if request.method == "POST" and not te_error:
            form = ControlAssistenciaForm(
                request.POST, prefix=str(control_a.pk), instance=control_a
            )
            ...
            form = ControlAssistenciaForm(prefix=str(control_a.pk), instance=control_a)
        ...
        activitat = (" - Activitat: " + str(sortida[0].titol)) if sortida else ""
        ...
        missatge = ""
        if (
            settings.CUSTOM_MOSTRAR_MAJORS_EDAT
            and alumne.edat(control_a.impartir.dia_impartir) >= 18
        ):
            missatge = settings.CUSTOM_MARCA_MAJORS_EDAT

        form.fields["estat"].label += missatge + (
            "(fa anys en aquesta data)" if avui_es_aniversari else ""
        )
    return form
...
def passaLlistaGrupDataTriaGrupDia(request):
    if request.method == "POST":
        ...
        if frm.is_valid():
            return HttpResponseRedirect(
                "/presencia/passaLlistaGrupData/{0}/{1}/{2}/{3}/".format(
    ...
    return render(
        ...
        {
            "form": frm,
            "head": "Passa llista a grup. Tria grup i data",
        },
    )
...
def passaLlistaGrupData(request, grup, dia, mes, year):
    ...
    controls = ControlAssistencia.objects.filter(
        alumne__grup=grup, impartir__dia_impartir=data
    ...
    if bool(formSet.forms):
        ...
        for f in formSet:
            ...
            AlumneNomSentit.objects.get(pk=f.instance.alumne.pk)
            f.fields["estat"].label = "{0} {1}".format(
    ...
    if request.method == "POST":
        ...
        if form.is_valid() and form.cleaned_data["marcar_com_hora_sense_alumnes"]:
            ...
            Accio.objects.create(
                ...
                l4=l4,
                impersonated_from=request.user if request.user != user else None,
                text="""Marcar classe sense alumnes {0}""".format(impartir),
            )
...
def afegeixAlumnesLlista(request, pk):
    ...
    if request.method == "POST":
        ...
        for grup in grups_a_mostrar:
            queryset = AlumneNomSentit.objects.filter(grup=grup).exclude(
                pk__in=alumnes_pk
            )
        ...
        if totBe:
            ...
            Accio.objects.create(
                ...
                l4=l4,
                impersonated_from=request.user if request.user != user else None,
                text="""Posar alumnes de la classe {0} (Forçat:{1}, Expandir:{2}): {3}""".format(
                    impartir,
                    "Sí" if matmulla else "No",
                    "Sí" if expandir else "No",
                    ", ".join([unicode(a) for a in alumnes]),
                ),
            )
        ...
        for grup in grups_a_mostrar:
            ...
            form = afegeixTreuAlumnesLlistaForm(
                prefix=str(grup.pk), queryset=queryset, etiqueta=unicode(grup)
            )
            formset.append(form)
...
def treuAlumnesLlista(request, pk):
    ...
    if request.method == "POST":
        ...
        #
        formExpandir = afegeixAlumnesLlistaExpandirForm(request.POST, prefix="tots")
        formExpandir.fields["matmulla"].help_text = (
            """Marca aquesta opció per treure alumnes tot i que hagi passat llista (només els absents)"""
        )
        formExpandir.fields["matmulla"].label = "Força treure"

        #
        # altres forms: grups d'alumnes
        #
        queryset = AlumneNomSentit.objects.filter(
        ...
        if form.is_valid() and formExpandir.is_valid():
            ...
            Accio.objects.create(
                ...
                l4=l4,
                impersonated_from=request.user if request.user != user else None,
                text="""Treure alumnes de la classe {0} (Forçat:{1}, Expandir:{2}): {3}""".format(
                    impartir,
                    "Sí" if matmulla else "No",
                    "Sí" if expandir else "No",
                    ", ".join([unicode(a) for a in alumnes]),
        ...
        form = afegeixTreuAlumnesLlistaForm(
            prefix=str("alumnes"), queryset=queryset, etiqueta="Alumnes a treure:"
        )
        formset.append(form)
...
def afegeixGuardia(request, dia=None, mes=None, year=None):
    ...
    url_next = "/presencia/mostraImpartir/%d/%d/%d/" % (int(year), int(mes), int(dia))
    if request.method == "POST":
...
def esborraGuardia(request, pk):
    ...
    # seg-------------------------------
    pertany_al_professor = (impartir.professor_guardia is not None) and (
        user.pk == impartir.professor_guardia.pk
    )
    if not (l4 or pertany_al_professor):
        return HttpResponseRedirect(url_next)
...
def calculadoraUnitatsFormatives(request):
    ...
    head = "Calculadora Unitats Formatives"
    infoForm = []

    grupsProfessor = (
        Grup.objects.filter(horari__professor=professor).order_by("curs").distinct()
    )
    assignaturesProfessor = (
        Assignatura.objects.filter(
            horari__professor=professor, horari__grup__isnull=False
        )
        .order_by("curs", "nom_assignatura")
        .distinct()
    )
        ...
        if form.is_valid():
            ...
            if imparticionsAssignatura.count() < hores:
                form._errors.setdefault(NON_FIELD_ERRORS, []).extend(
                    [
                        """A partir de la data {0} només hi ha {1} hores,
                                   comprova que has triat bé el curs.
                                   """.format(
                            dataInici, imparticionsAssignatura.count()
                        )
                    ]
                )
            else:
    ...
    return render(
        request,
        "form.html",
        {"form": form, "infoForm": infoForm, "head": head},
    )
...
def indicadors(request):
    ...
    return render(
        request,
        "report.html",
        {"report": report, "head": "Indicadors", "menuCTX": menuCTX},
    )
...
def faltesAssistenciaEntreDates(request):
    ...
    head = "Calculadora %assistència entre Dates"
...
def copiarAlumnesLlista(request, pk):
    ...
    dInici = date(
        year=dillunsSetmana.year, month=dillunsSetmana.month, day=dillunsSetmana.day
    )
    dFi = date(
        year=divendresSetmana.year,
        month=divendresSetmana.month,
        day=divendresSetmana.day,
    )

    horarisProfe = (
        Impartir.objects.filter(dia_impartir__gte=dInici)
    ...
    for eH in horarisProfe:
        ...
        opcions.append(
            (
                eH.id,
                unicode(eH.horari.assignatura)
                + " "
                + unicode(eH.horari.dia_de_la_setmana)
                + " "
                + unicode(eH.horari.hora)
                + "--> Alumnes: "
        ...
        if formHores.is_valid():
            ...
            if int(idHoraOrigen) == int(idHoraDesti):
                formHores._errors.setdefault(NON_FIELD_ERRORS, []).extend(
                    ["""No pots copiar alumnes sobre la mateixa hora."""]
                )
            else:
                ...
                if eliminarAlumnes:
                    treu = treuThread(
                        expandir=None,
                        alumnes=list(alumnesDesti.values()),
                        impartir=horaDesti,
                        matmulla=False,
                        usuari=user,
                    )
                    ...
                    Accio.objects.create(
                        ...
                        text="""Copiar alumnes, eliminar alumnes de {0}: {1}""".format(
                            horaDesti, ", ".join([unicode(a) for a in alumnesDesti])
                        ),
                    )
                ...
                Accio.objects.create(
                    ...
                    l4=l4,
                    impersonated_from=request.user if request.user != user else None,
                    text="""Copiar alumnes de {0} a {1}: {2}""".format(
                        impartir,
                        horaDesti,
                        ", ".join([unicode(a) for a in alumnesAAfegir]),
                    ),
                )
...
def anularImpartir(request, pk):
    ...
    if not controls.exists():
        messages.error(request, "Aquesta classe no té alumnes, no es pot anul·lar.")
        ...
        for control in (
            controls.exclude(q_already_anulats)
            .filter(q_sense_passar_llista | q_falta)
            .all()
        ):
        ...
        if not bool(errors):
            ...
            messages.error(
                request,
                "S'han trobat errors anul·lant aquesta hora de classe: {}".format(
                    ", ".join(errors)
                ),
            )
...
def desanularImpartir(request, pk):
    ...
    if not controls.exists():
        messages.error(
            request,
            "Aquesta classe no té alumnes, vols dir que és aquesta la que vols des-anul·lar?",
        )
        next_url = reverse("aula__horari__passa_llista", kwargs={"pk": pk})
    elif not NoHaDeSerALAula.objects.filter(
        control__impartir=impartir, motiu=NoHaDeSerALAula.ANULLADA
    ).exists():
        messages.error(
            request,
            "Aquesta classe no està anul·lada, vols dir que és aquesta la classe que vols des-anul·lar?",
        )
        next_url = reverse("aula__horari__passa_llista", kwargs={"pk": pk})
    else:
        errors = []
        ...
        if not bool(errors):
            messages.success(request, "Operació finalitzada. Classe des-anul·lada.")
        else:
            messages.error(
                request,
                "S'han trobat errors des-anul·lant aquesta hora de classe: {}".format(
                    ", ".join(errors)
                ),
            )
        next_url = reverse("aula__horari__passa_llista", kwargs={"pk": pk})
...
def winwheel(request, pk):
    ...
    def es_present(c):
        return c is None or c.estat is None or c.estat.codi_estat != "F"

    alumnes_i_presencialitat = [
        (c.alumne, es_present(c))
        for c in impartir.controlassistencia_set.all()
        if (not c.alumne.esBaixa() and not c.nohadeseralaula_set.exists())
    ]

    # nom_i_inicials: No hi ha gaire espai a la ruleta, pintem 'Laia B.C.'
    def nom_i_inicials(a):
        return (
            (a.nom_sentit or a.nom)
            + " "
            + ".".join([x[:1] for x in a.cognoms.split(" ")])
            + "."
        )
    ...
    def tria_color(idx, present):
        return color_falta if not present else colors[idx % 4]
