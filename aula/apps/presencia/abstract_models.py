...
class AbstractImpartir(models.Model):
    ...
    professor_guardia = models.ForeignKey(
        "usuaris.Professor",
        null=True,
        blank=True,
    ...
    professor_passa_llista = models.ForeignKey(
        "usuaris.Professor",
        null=True,
        blank=True,
        db_index=True,
    ...
    reserva = models.ForeignKey(
        "aules.ReservaAula", null=True, blank=True, on_delete=models.SET_NULL
    )
    ...
    def __str__(self):
        ...
        if self.canvi_aula_respecte_horari:
            canviaula = " amb canvi d'aula a la {aula}".format(aula=self.get_nom_aula)
        resposta = "{dia}: {horari}{canviaula}".format(
    ...
    def get_nom_aula(self):
        ...
        if te_reserva:
            ...
            return "{aula}{alarma}".format(
                aula=self.reserva.aula.nom_aula, alarma=alarma
            )
        else:
    ...
    def resum(self):
        ...
        nIncidencies = self.controlassistencia_set.filter(
            incidencia__isnull=False
        ).count()
        nExpulsions = self.controlassistencia_set.filter(expulsio__isnull=False).count()
        return "Al:{0} In:{1} Ex:{2}".format(nAlumnes, nIncidencies, nExpulsions)

    def hi_ha_nulls(self):
        return (
            self.controlassistencia_set.filter(estat__isnull=True)
    ...
    def hi_ha_alumnes_amb_activitat_programada(self):
        #
        q_sortida_comenca_mes_tard = Q(
            controlassistencia__alumne__sortides_confirmades__data_inici__gt=self.dia_impartir
        ) | (
            Q(
                controlassistencia__alumne__sortides_confirmades__franja_inici__hora_inici__gt=self.horari.hora.hora_inici
            )
            & Q(
                controlassistencia__alumne__sortides_confirmades__data_inici=self.dia_impartir
            )
        )
        q_sortida_acaba_abans = Q(
            controlassistencia__alumne__sortides_confirmades__data_fi__lt=self.dia_impartir
        ) | (
            Q(
                controlassistencia__alumne__sortides_confirmades__franja_fi__hora_fi__lt=self.horari.hora.hora_fi
            )
            & Q(
                controlassistencia__alumne__sortides_confirmades__data_inici=self.dia_impartir
            )
        ...
        hi_ha_alumnes_a_la_sortida = False and (
            self.__class__.objects.filter(~q_fora_de_rang & q_es_meu).exists()
        )
...
class AbstractEstatControlAssistencia(models.Model):
    ...
    pct_ausencia = models.IntegerField(
        default=0,
        null=False,
        blank=False,
        help_text="100=Falta tota l'hora, 0=No és falta assistència. Aquest camp serveix per que els retrassos es puguin comptar com a falta o com un percentatge de falta.",
    )
...
class AbstractControlAssistencia(models.Model):
    alumne = models.ForeignKey(
        to="alumnes.Alumne", db_index=True, on_delete=models.CASCADE
    )
    estat = models.ForeignKey(
        "presencia.EstatControlAssistencia",
        db_index=True,
        null=True,
        blank=True,
        on_delete=models.CASCADE,
    )
    impartir = models.ForeignKey(
        "presencia.Impartir", db_index=True, on_delete=models.CASCADE
    )
    professor = models.ForeignKey(
        "usuaris.Professor", null=True, blank=True, on_delete=models.CASCADE
    )

    swaped = models.BooleanField(default=False)
    estat_backup = models.ForeignKey(
        "presencia.EstatControlAssistencia",
        related_name="controlassistencia_as_bkup",
        db_index=True,
        null=True,
        blank=True,
        on_delete=models.CASCADE,
    )
    professor_backup = models.ForeignKey(
        "usuaris.Professor",
        related_name="controlassistencia_as_bkup",
        null=True,
        blank=True,
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["alumne", "estat"]),
        ]

    def __str__(self):
        return unicode(self.alumne) + " -> " + unicode(self.estat)
    ...
    @property
    def descripcio(self):
        text = "{0}:{1}  Prof.: {2}".format(
            self.estat or "",
            self.impartir.horari.assignatura if self.impartir.horari else "",
            self.professor
            or (self.impartir.horari.professor if self.impartir.horari else ""),
        )
        if self.comunicat:
...
class AbstractNoHaDeSerALAula(models.Model):
    ...
    MOTIUS_CHOICE = (
        ...
        (ANULLADA, "Classe Anul·lada"),
    )

    motiu = models.CharField(max_length=5, choices=MOTIUS_CHOICE)
    control = models.ForeignKey(
        to="presencia.ControlAssistencia", on_delete=models.CASCADE, db_index=True
    )
    sancio = models.ForeignKey(
        to="incidencies.Sancio",
        blank=True,
        null=True,
        on_delete=models.CASCADE,
        db_index=True,
    )
    sortida = models.ForeignKey(
        to="sortides.Sortida",
        blank=True,
        null=True,
        on_delete=models.CASCADE,
    ...
    class Meta:
        abstract = True
        verbose_name = "Motiu no ha està pressent"
        verbose_name_plural = "Motius no presència a l'aula"

    def __str__(self):
        return unicode(self.control.alumne) + " -> " + unicode(self.get_motiu_display())
