...
from aula.apps.missatgeria.missatges_a_usuaris import MAIL_REBUTJAT, tipusMissatge
...
def geturlconf(tipus, usuari, responsable=None):
    ...
    if tipus == "ADM":
        if responsable:
            url = (
                "/admin/relacioFamilies/responsable/{0}/change/".format(responsable.pk)
                if responsable.pk
                else ""
            )  # administrador per responsable
        elif al:
            url = (
                "/admin/alumnes/alumne/{0}/change/".format(codi) if codi else ""
            )  # administrador per alumne
        else:
            url = (
                "/admin/auth/user/{0}/change/".format(codi) if codi else ""
            )  # administrador per usuari no alumne
    else:
        if tipus == "TUT" and (al or responsable):
            url = (
                "/open/configuraConnexio/{0}/".format(codi) if codi else ""
            )  # tutor per modificar alumne o responsable
        else:
...
def informa(emailRetornat, status, action, data, diagnostic, text):
    ...
    correus = Q(correu=emailRetornat) | Q(
        responsables__correu_relacio_familia=emailRetornat
    )
    ...
    if altres.exists():
        ...
        for usuari in altres:
            informaDSN(
                usuari, usuari, emailRetornat, motiu, data, geturlconf("USU", usuari)
            )
            informaDSN(
                administradors,
                usuari,
    ...
    if not alumnes.exists() and not altres.exists():
        ...
        informaDSN(
            administradors, None, emailRetornat, motiu, data, geturlconf("ADM", None)
        )
...
def enviaOneTimePasswdAlumne(alumne, email, force=False):
    ...
    alumne.get_user_associat().is_active
    ...
    if total_enviats >= 3:
        ...
        warnings.append("Aquest usuari és baixa. No se li pot enviar codi d'accés.")
        ...
        txtCapcelera = "Enviat missatge a {0} .".format(email)
        ...
        try:
            ...
            send_mail(
                assumpte, "\n".join(missatge), fromuser, [email], fail_silently=False
            )
...
def enviaBenvingudaAlumne(alumne, force=False):
    ...
    if not correusFamilia:
        if alumne.edat() < 18:
            warnings.append(
                "Comprova que l'adreça electrònica d'almenys un dels responsables estigui informada"
            )
        else:
            warnings.append(
                "Comprova que l'adreça electrònica de l'alumne o un dels responsables estigui informada"
            )
        ...
        # Tenim una variable: settings.URL_DJANGO_AULA
        # que potser s'hauria d'incloure al missatge

        # Tenim una variable: settings.CUSTOM_PORTAL_FAMILIES_TUTORIAL
        # que potser s'hauria d'incloure al missatge
...
def ultimaNotificacio(usuari, alumne):
    ...
    if usuari:
        ultima = (
            NotifUsuari.objects.filter(usuari=usuari, alumne=alumne, tipus="N")
            .order_by("-moment")
            .first()
        )
    else:
        ultima = (
            NotifUsuari.objects.filter(alumne=alumne, tipus="N")
            .order_by("-moment")
            .first()
        )
    if ultima:
...
def set_notificacio(element, notificacio, tipus="N"):
    if not notificacio:
        return
    notifs = element.notificacions_familia.filter(
        usuari=notificacio.usuari, tipus=tipus
    )
    if notifs:
        for n in notifs:
            element.notificacions_familia.remove(n)
...
def get_notif_revisio(element, usuari, fmt_data=None):
    ...
    if User2Professor(usuari):
        ...
        revis = (
            element.notificacions_familia.filter(tipus="R").order_by("moment").first()
        )
    else:
        notif = (
            element.notificacions_familia.filter(usuari=usuari, tipus="N")
            .order_by("-moment")
            .first()
        )
        revis = (
            element.notificacions_familia.filter(usuari=usuari, tipus="R")
            .order_by("-moment")
            .first()
        )
    if notif:
        notif = notif.moment.strftime(fmt_data)
    else:
        notif = notifc
    if revis:
        revis = revis.moment.strftime(fmt_data)
    else:
        revis = revisc
    return notif, revis
