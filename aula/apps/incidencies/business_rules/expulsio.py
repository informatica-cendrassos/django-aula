...
def expulsio_clean(instance):
    ...
    if instance.dia_expulsio is None or instance.franja_expulsio_id is None:
        errors.setdefault("dia_expulsio", []).append("Falten Dades: Cal dia i franja.")
        errors.setdefault("franja_expulsio", []).append(
            "Falten Dades: Cal dia i franja."
        )
    ...
    if instance.tramitacio_finalitzada and not instance.tutor_contactat_per_l_expulsio:
        errors.setdefault("tutor_contactat_per_l_expulsio", []).append(
            "Falten Dades: tutor contactat."
        )
    ...
    if instance.tramitacio_finalitzada and not instance.moment_comunicacio_a_tutors:
        errors.setdefault("moment_comunicacio_a_tutors", []).append(
            "Falten Dades: moment de contacte."
        )

    # si tramitacio_finalitzada la data ha de ser inferior a ara
    if (
        instance.tramitacio_finalitzada
        and instance.moment_comunicacio_a_tutors
    ...
    correspon_usuari = (
        instance.professor.pk == user.pk if user and instance.professor else True
    )
