...
def incidencia_clean(instance):
    ...
    if instance.es_incidencia_d_aula() and not instance.gestionada_pel_tutor:
        ...
        la_posa_el_professor_de_guardia = (professor is not None) and (
        ...
        if la_posa_el_professor_de_guardia:
            ...
            instance.gestionada_pel_tutor_motiu = (
                Incidencia.GESTIONADA_PEL_TUTOR_GUARDIA
            )
    ...
    if (
        ...
        and instance.tipus is not None
        and instance.tipus.es_informativa
    ):
    ...
    if not instance.dia_incidencia:
        errors.setdefault("dia_incidencia", []).append("Falten Dades: Cal dia")

    # dia i hora sempre informats
    if not instance.franja_incidencia_id:
        errors.setdefault("franja_incidencia", []).append("Falten Dades: Cal franja")
    ...
    if instance.dia_incidencia and instance.dia_incidencia < (
        dt.date.today() + dt.timedelta(days=-periode)
    ):
        errors.setdefault("dia_incidencia", []).append(
            """No es poden posar o modificar incidències amb més de {0} dies.""".format(
                periode
            )
        )
    ...
    if (
        ...
        and instance.franja_incidencia_id
        and instance.getDate() > datetime.now()
    ):
        errors.setdefault("dia_incidencia", []).append(
            """Encara no pots posar incidències en aquesta classe. Encara no s'ha realitzat."""
        )

    # No incidencies alumne que és baixa:
    if (
        instance.alumne.data_baixa is not None
        and instance.alumne.data_baixa < instance.dia_incidencia
    ):
        errors.setdefault("dia_incidencia", []).append(
            """L'alumne estava de baixa en aquesta data."""
        )
...
def Incidencia_pre_delete(sender, instance, **kwargs):
    ...
    if instance.dia_incidencia < (dt.date.today() + dt.timedelta(days=-30)):
        errors[NON_FIELD_ERRORS] = [
            """Aquesta incidència és massa antiga per ser esborrada (Té més d' una setmana)"""
        ]
    ...
    if user and not es_meva:
        errors[NON_FIELD_ERRORS] = [
            """No tens permisos per esborrar aquesta incidència."""
        ]
    ...
    if not instance.tipus.es_informativa:
        if settings.CUSTOM_INCIDENCIES_PROVOQUEN_EXPULSIO:
            ...
            if nIncidenciesAlumneProfessor > 2:
                ...
                msg = Missatge(
                    remitent=instance.professional.getUser(),
                    text_missatge=txt,
        ...
        if bool(instance.professional_inicia):
            ...
            text_missatge = missatge.format(
                "Incidència de Retard d'entrada al centre",
                instance.alumne,
        ...
        if instance.tipus.notificar_equip_directiu:
            ...
            Professor = apps.get_model("usuaris", "Professor")
            membres_equip_directiu = Professor.objects.filter(groups__name="direcció")
