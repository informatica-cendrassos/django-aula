...
def imprimir(request, pk, din="4"):
    ...
    fEsAdministratiu = request.user.groups.filter(name__in=["administratius"]).exists()

    potEntrar = (
        professor in instance.professors_responsables.all()
    ...
    barres = codi_barres.save(
        "/tmp/barcode-{0}-{1}".format(time.time(), request.session.session_key)
    )
    ...
    for alumne in Alumne.objects.filter(pk__in=[a.id for a in alumnes]):
        ...
        o.departament = (
            unicode(instance.departament_que_organitza)
            if instance.departament_que_organitza
            else instance.comentari_organitza
        )
        ...
        o.terminipagament = (
            ""
            if not bool(instance.termini_pagament)
            else "- darrer dia pagament {0} -".format(
    ...
    if True:  # not excepcio:
        ...
        response["Content-Disposition"] = (
            'attachment; filename="autoritzacio-{0}-{1}.odt"'.format(
                dict(Sortida.TIPUS_ACTIVITAT_CHOICES)[instance.tipus], pk
            )
        )
...
def sortidesMevesList(request, tipus="A"):
    ...
    sortides = Sortida.objects.filter(
        q_professor_proposa | q_professors_responsables | q_professors_acompanyants
    ).distinct()
    if tipus:
...
def sortidesGestioList(request, tipus=None):
    ...
    socSecretari = User.objects.filter(
        pk=user.pk, groups__name__in=["secretaria"]
    ).exists()
    socAdministratiu = User.objects.filter(
        pk=user.pk, groups__name__in=["administratius"]
    ).exists()
    ...
    sortides = Sortida.objects.filter(estat__in=filtre).distinct()
    if tipus:
        sortides = sortides.filter(tipus=tipus)
...
def sortidesConsergeriaList(request):
    ...
    sortides = list(
        Sortida.objects.filter(
            calendari_finsa__gte=avui, estat__in=["R", "G"]
        ).distinct()
    )
...
def sortidaEdit(request, pk=None, clonar=False, origen=False, tipus="A"):
    ...
    if bool(pk) and not clonar:
        ...
        instance.estat = "E"
        instance.titol = (
            "**CLONADA** " + "SECRETARÍA**" if fEsAdministratiu else "" + instance.titol
        )
    ...
    #         instance.professor_que_proposa_id = None
        ...
        instance = Sortida()
        instance.professor_que_proposa = professor
        instance.tipus_de_pagament = (
            "ON" if settings.CUSTOM_SORTIDES_PAGAMENT_ONLINE else "NO"
        )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            if form.cleaned_data["tipus_de_pagament"] == "NO":
                instance.preu_per_alumne = 0
                ...
                if tipus != "P":
                    ...
                    primeraimparticio = (
                        Impartir.objects.filter(
                            dia_impartir=instance.calendari_desde.date(),
                            horari__hora__hora_inici__gte=instance.calendari_desde.time(),
                        )
                        .order_by("dia_impartir", "horari__hora__hora_inici")
                        .first()
                    )
                    primerafranja = (
                        primeraimparticio.horari.hora if primeraimparticio else None
                    )
                    ...
                    darreraimparticio = (
                        Impartir.objects.filter(
                            dia_impartir=instance.calendari_finsa.date(),
                    ...
                    darrerafranja = (
                        darreraimparticio.horari.hora if darreraimparticio else None
                    )
                    ...
                    if (
                        instance.data_fi
                        and instance.data_inici
                        and instance.data_fi < instance.data_inici
                    ):
            ...
            if origen == "Meves":
                safetext = (
                    """RECORDA: Una vegada enviades les dades, 
                                                  has de seleccionar els <a href="{0}">alumnes implicats</a>
                                                  des del menú desplegable ACCIONS""".format(
                        "/sortides/alumnesConvocats/{id}/{tipus}".format(
                            id=instance.id, tipus=tipus
                        ),
                    )
                    if tipus == "P"
                    else """RECORDA: Una vegada enviades les dades,
                        ...
                        "/sortides/alumnesFallen/{id}/{tipus}".format(
                            id=instance.id, tipus=tipus
                        ),
                    )
                )
                messages.warning(request, SafeText(safetext))
            ...
            acompanyen_nous = (
                professors_acompanyen_despres - professors_acompanyen_abans
            )
            organitzen_nous = (
                professors_organitzen_despres - professors_organitzen_abans
            )

            # helper missatges:
            data_inici = "( data activitat encara no informada )"
            ...
            if instance.tipus != "P":
                ...
                enllac = reverse(
                    ...
                    kwargs={"pk": instance.id, "tipus": instance.tipus},
                )
            ...
            tipus_de_missatge = tipusMissatge(missatge)
            msg = Missatge(
                remitent=user, text_missatge=txt, tipus_de_missatge=tipus_de_missatge
            )
    ...
    if tipus != "P":
        ...
        form.fields["calendari_public"].widget.attrs["style"] = "width: 3%"
    ...
    if not fEsDireccioOrGrupSortides:
        ...
        if not settings.CUSTOM_FORMULARI_SORTIDES_REDUIT:
            form.fields["codi_de_barres"].widget.attrs["disabled"] = "disabled"
        form.fields["informacio_pagament"].widget.attrs["disabled"] = "disabled"
        form.fields["preu_per_alumne"].widget.attrs["disabled"] = "disabled"

    # si no és propietari tot a disabled
    deshabilitat = instance.id and not (
        professor in instance.professors_responsables.all()
        or fEsDireccioOrGrupSortides
        or fEsAdministratiu
    )
    ...
    if deshabilitat:
        ...
        form.fields["estat"].label += ": {0} ".format(instance.get_estat_display())
...
def alumnesConvocats(request, pk, origen, tipus=None):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                    ...
                    try:
                        ...
                        if instance.tipus_de_pagament == "ON":
                            ...
                            pag = SortidaPagament.objects.filter(
                                alumne=alumne, sortida=instance
                            )
                            if not pag:
                ...
                nexturl = r"/sortides/sortides{origen}/{tipus}".format(
                    origen=origen, tipus=instance.tipus
                )
                return HttpResponseRedirect(nexturl)
    ...
    q_base = AlumneGrupNom.objects.order_by(
        "grup__curs__nivell__ordre_nivell",
...
def alumnesFallen(request, pk, origen, tipus=None):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                ante = set(
                    [x.pk for x in instance.alumnes_que_no_vindran.all().distinct()]
                )
                # afegir
                for alumne in nous - ante:
                    try:
    ...
    form.fields["alumnes_que_no_vindran"].queryset = AlumneGrupNom.objects.filter(
...
def professorsAcompanyants(request, pk, origen, tipus=None):
    ...
    formIncidenciaF = modelform_factory(
        Sortida,
        fields=("altres_professors_acompanyants",),
        widgets={
            "altres_professors_acompanyants": ModelSelect2MultipleWidget(
                queryset=Professor.objects.all(),
                search_fields=(
    ...
    # si no és propietari tot a disabled
    deshabilitat = instance.id and not (
        professor in instance.professors_responsables.all() or fEsDireccioOrGrupSortides
    )
...
def esborrar(request, pk, origen):
    ...
    potEntrar = (
        mortalPotEntrar
        or direccio
        or (esAdministratiu and "SECRETARÍA" in instance.titol)
    )
    if not potEntrar:
        messages.warning(request, "No pots esborrar aquest/a pagament/activitat.")
...
def sortidaiCal(request):
    ...
    for instance in (
        Sortida.objects.filter(calendari_desde__isnull=False)
        .exclude(
            estat__in=[
                "E",
                "P",
        ...
        summary = "{ambit}: {titol}".format(ambit=instance.ambit, titol=instance.titol)

        event.add("dtstart", localtime(instance.calendari_desde))
        event.add("dtend", localtime(instance.calendari_finsa))
        event.add("summary", summary)
        organitzador = "\nOrganitza: "
        organitzador += "{0}".format(
            "Departament" + instance.departament_que_organitza.nom
            if instance.departament_que_organitza_id
            else ""
        )
...
def sortidaExcel(request, pk):
    ...
    fEsAdministratiu = request.user.groups.filter(name__in=["administratius"]).exists()
    potEntrar = (
        professor in sortida.professors_responsables.all()
    ...
    if sortida.tipus != "P":
        detall += [
            ...
            [
                sortida.calendari_desde.strftime("%d/%m/%Y %H:%M"),
                sortida.calendari_finsa.strftime("%d/%m/%Y %H:%M"),
            ],
            [],
        ]
    detall += (
        ...
        + [[unicode(p)] for p in sortida.professors_responsables.all()]
        + [[]]
    )
    if sortida.tipus != "P":
        detall += (
            ...
            + [[unicode(p)] for p in sortida.altres_professors_acompanyants.all()]
            + [[]]
        )
    ...
    if sortida.tipus != "P":
        alumnes[0].extend(["Assistència"])
    if sortida.tipus_de_pagament == "ON":
        alumnes[0].extend(["Pagat", "Preu", "Data Pagament", "Codi Pagament"])
    ...
    for alumne in sortida.alumnes_convocats.all():
        row = [
            ...
            (
                alumne.dadesaddicionalsalumne_set.get(label="Drets imatge").value
                if alumne.dadesaddicionalsalumne_set.filter(
                    label="Drets imatge"
                ).exists()
                else ""
            ),
            (
                alumne.dadesaddicionalsalumne_set.get(label="Dades mèdiques").value
                if alumne.dadesaddicionalsalumne_set.filter(
        ...
        if sortida.tipus_de_pagament == "ON" and alumne not in novindran:
            ...
            ordre = (
                pagament.ordre_pagament
                if pagament.ordre_pagament and pagament.pagament_realitzat
                else ""
            )
            observacions = (
                pagament.observacions
                if pagament.observacions and pagament.pagament_realitzat
                else ""
            )
            row.extend(
                [
                    "Si" if pagament_realitzat else "No",
                    pagament.sortida.preu_per_alumne if pagament_realitzat else "",
                    (
                        pagament.data_hora_pagament.strftime("%d/%m/%Y %H:%M")
                        if pagament_realitzat
                        else ""
                    ),
    ...
    if True:  # not excepcio:
        ...
        response["Content-Disposition"] = 'attachment; filename="{0}-{1}.xlsx"'.format(
            dict(Sortida.TIPUS_ACTIVITAT_CHOICES)[sortida.tipus], slugify(sortida.titol)
        )
...
def logPagaments(txt, tipus="ADMINISTRACIO"):
    ...
    """
    msg = Missatge(
        remitent=User.objects.filter(groups__name__contains="administradors").first(),
        text_missatge=txt,
        tipus_de_missatge=tipus,
    )
    importancia = "VI"
...
def pagoOnlineBase(request, pk):
        ...
        return render(
            ...
            "resultat.html",
            {
                "msgs": {
                    ...
                    "warnings": [],
                    "infos": [
                        "AQUEST USUARI NO TÉ AUTORITZACIÓ PER ACCEDIR AL PAGAMENT."
                    ],
                }
            },
        )
                ...
                return render(
                    ...
                    "resultat.html",
                    {
                        "msgs": {
                            ...
                            "warnings": [],
                            "infos": [
                                "PAGAMENT JA OBERT EN UN ALTRE CONNEXIÓ. CANCEL·LA O ESPERA UNS MINUTS."
                            ],
                        }
                    },
                )
    ...
    if request.method == "POST":
        ...
        form = PagamentForm(initial={"sortida": sortida, "acceptar_condicions": False})
    return render(
        request,
        "formPagamentOnline.html",
        {
            ...
            "sortida": (
                sortida
                if pagament.sortida
                else descripcio_sortida + "(" + str(pagament.quota.any) + ")"
            ),
            "descripcio": descripcio_sortida,
            "preu": preu,
...
def passarella(request, pk):
    ...
    cipher = DES3.new(
        key=base64.b64decode(keyComerç), mode=DES3.MODE_CBC, IV=b"\0\0\0\0\0\0\0\0"
    )
    ...
    try:
        ...
        if int(ds_response) in range(0, 100):
            ...
            if not pagament.alumne:
                ...
                pagament.alumne = Alumne.objects.get(
                    pk=int(pagament.ordre_pagament[:-7])
                )
                logPagaments(
                    "Pagament caducat pasa a ok: "
                    + str(pagament.pk)
                    + " alumne: "
                    + str(pagament.alumne.id)
                )
            pagament.save()
        else:
            """
        ...
        txt = (
            ...
            + "\n"
            + str(e)
            + " apps.sortides.views.retornTransaccio"
        )
        logPagaments(txt)
...
def pagoEfectiu(request, pk):
    ...
    preu = sortida.preu_per_alumne
    alumne = pagament.alumne
    fEsGrupSecrretaria = request.user.groups.filter(name__in=["secretaria"]).exists()
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                ...
                return HttpResponseRedirect(
                    reverse(
                        "sortides__sortides__detall_pagament", kwargs={"pk": sortida.pk}
                    )
                )
            except ValidationError as e:
    ...
    return render(
        ...
        {
            "form": form,
            "head": "Pagament Efectiu",
            "titol_formulari": "Es realitza el pagament en efectiu d'una activitat programada per pagar online",
        },
    )
...
def detallPagament(request, pk, tipus=None):
    ...
    head = "{0}: {1}  ({2} €)".format(
        dict(Sortida.TIPUS_ACTIVITAT_CHOICES)[sortida.tipus],
        sortida.titol,
        str(sortida.preu_per_alumne),
    )
    ...
    capcelera = tools.classebuida()
    capcelera.amplade = 40
    capcelera.contingut = "Pagat"
    taula.capceleres.append(capcelera)

    capcelera = tools.classebuida()
    capcelera.amplade = 20
    capcelera.contingut = "Codi Pagament"
    taula.capceleres.append(capcelera)
    ...
    for pagament in SortidaPagament.objects.filter(sortida=sortida).order_by("alumne"):
        ...
        camp.contingut = (
            pagament.data_hora_pagament.strftime("%d/%m/%Y %H:%M")
            if pagament.data_hora_pagament and pagament.pagament_realitzat
            else "No"
        )
        filera.append(camp)
        ...
        # -Efectiu--------------------------------------------
        fEsGrupSecrretaria = request.user.groups.filter(
            name__in=["secretaria"]
        ).exists()
        if fEsGrupSecrretaria:
            camp = tools.classebuida()
            camp.enllac = (
                "/sortides/pagoEfectiu/{0}/".format(pagament.pk)
                if not pagament.pagament_realitzat
                else None
            )
            camp.contingut = (
                """Pagament/Efectiu""" if not pagament.pagament_realitzat else ""
            )
            filera.append(camp)
    ...
    if request.method == "POST":
        ...
        if formset.is_valid():
            ...
            for form in formset:
                ...
                try:
                    pagament = (
                        QuotaPagament.objects.get(pk=pkp) if pkp != "None" else None
                    )
                        ...
                        if pagament.quota != quota or pagament.fracciona != fracciona:
                            ...
                            if p:
                                if not p.filter(pagament_realitzat=True):
                                    fraccions_esborrades = fraccions_esborrades + tuple(
                                        p.values_list("pk", flat=True)
                                    )
                                    p.delete()
                                else:
            ...
            if modifFracc:
                messages.warning(request, "No es pot modificar un fraccionament pagat.")
            if modifPag:
                messages.warning(
                    request, "No es pot modificar un pagament ja completat."
                )
            if esbFracc:
                messages.warning(request, "No es pot esborrar un fraccionament pagat.")
            if esbPag:
                messages.warning(
                    request, "No es pot esborrar un pagament ja completat."
                )
            if canviSim:
                messages.warning(
                    request,
                    "Un altre usuari també fa canvis a {0}".format(str(a.grup.curs)),
                )

            llista = Alumne.objects.filter(
                grup__curs__id=curs,
                ...
                return render(
                    ...
                    "resultat.html",
                    {
                        "msgs": {
                            "errors": [],
                            "warnings": [],
                            "infos": ["Sense quotes assignades"],
                        }
                    },
                )
            formsetQuotes = formset_factory(PagQuotesForm, extra=0)
            formset = formsetQuotes(
                form_kwargs={"tipus": tipus, "any": nany}, initial=llistapag
            )
            ...
            return render(
                ...
                "resultat.html",
                {
                    "msgs": {
                        "errors": [],
                        "warnings": [],
                        "infos": ["Sense quotes per assignar"],
                    }
                },
            )
...
def acumulatsQuotesCurs(tpv, nany=None):
    ...
    totfet = (
        QuotaPagament.objects.filter(
            quota__tpv=tpv, pagament_realitzat=True, data_hora_pagament__year=nany
        )
    ...
    totpendent = (
        QuotaPagament.objects.filter(
            quota__tpv=tpv, pagament_realitzat=False, alumne__isnull=False
        )
        .exclude(quota__curs__isnull=True)
        .values_list("quota")
        .annotate(total=Sum("quota__importQuota", filter=Q(fracciona=False)))
...
def acumulatsQuotesGen(tpv, nany=None):
    ...
    totfet = (
        ...
        .values_list(
            "alumne__grup__curs__nivell__nom_nivell",
            "quota__tipus__nom",
            "data_hora_pagament__month",
        )
        .annotate(total=Sum("quota__importQuota", filter=Q(fracciona=False)))
        .annotate(totalf=Sum("importParcial", filter=Q(fracciona=True)))
    )
...
def fullcalculQuotes(tpv, nany=None):
    ...
    totes = (
        Quota.objects.filter(importQuota__gt=0)
        .values_list("id")
        .order_by("curs__nom_curs_complert", "descripcio")
    )
    ...
    for q in totes:
        if q[0] in acumulats:
            ...
            worksheet.write(
                fila, 0, quota.descripcio + "(" + quota.curs.nom_curs_complert + ")"
            )
            for m, v in a.items():
                if m == "pendent":
                    col = 1
            ...
            worksheet.write_formula(
                fila, 14, "=SUM(C{0}:N{0})".format(fila + 1), num_format
            )
            fila = fila + 1
    ...
    if fila > 2:
        for t in range(ord("B"), ord("P")):
            worksheet.write_formula(
                fila, t - ord("A"), "=SUM({0}3:{0}{1})".format(chr(t), fila), num_format
            )

    worksheet = workbook.add_worksheet("Pendents")
    worksheet.set_column(0, 4, 30)
    worksheet.write_string(
        0, 0, "Pagaments pendents. Dades a " + date.strftime("%d/%m/%Y %H:%M")
    )
    worksheet.write_row(
        1, 0, ("Concepte", "Alumne", "Import", "Data límit", "Fraccionat")
    )
    fila = 2
    pagq = QuotaPagament.objects.filter(
        quota__tpv=tpv, pagament_realitzat=False, alumne__isnull=False
    ).order_by(
        "quota__dataLimit", "quota__descripcio", "alumne__cognoms", "alumne__nom"
    )
    # Només es permet afegir les activitats/sortides si tpv és el de defecte
    if not tpv or tpv.nom == "centre":
        pags = (
            SortidaPagament.objects.filter(
                pagament_realitzat=False,
                sortida__preu_per_alumne__isnull=False,
                alumne__isnull=False,
                sortida__notificasortida__alumne__isnull=False,
            )
            .distinct()
            .order_by(
                "sortida__termini_pagament",
                "sortida__titol",
                "alumne__cognoms",
                "alumne__nom",
            )
    ...
    for p in pagq:
        ...
        worksheet.write_number(
            fila, 2, p.quota.importQuota if not p.fracciona else p.importParcial
        )
        dataquota = p.quota.dataLimit if not p.fracciona else p.dataLimit
        if dataquota:
            worksheet.write_datetime(fila, 3, dataquota, date_format)
    ...
    for p in pags:
        if bool(p.sortida.preu_per_alumne) and p.sortida.preu_per_alumne > 0:
            ...
            if p.sortida.termini_pagament:
                worksheet.write_datetime(
                    fila, 3, p.sortida.termini_pagament, date_format
                )
            fila = fila + 1
