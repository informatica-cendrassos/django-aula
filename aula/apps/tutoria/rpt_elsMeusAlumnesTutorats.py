...
from aula.apps.tutoria.models import ResumAnualAlumne, SeguimentTutorialRespostes, Tutor
from aula.utils import tools
from aula.utils.tools import unicode


def elsMeusAlumnesTutoratsRpt(
    professor=None, grup=None, dataDesDe=None, dataFinsA=date.today()
):
    report = []
    ...
    for grup in grups:
        ...
        capcelera.contingut = "Disciplina"
        taula.capceleres.append(capcelera)
        ...
        capcelera = tools.classebuida()
        capcelera.amplade = 15
        capcelera.contingut = "Familia"
        taula.capceleres.append(capcelera)

        capcelera = tools.classebuida()
        capcelera.amplade = 10
        capcelera.contingut = "Seguiment"
        taula.capceleres.append(capcelera)
        ...
        for alumne in Alumne.objects.filter(consulta_alumnes):
            ...
            # -%--------------------------------------------
            tpc_injust = (
                (1.0 * f) * 100.0 / (0.0 + f + r + p + j) if (f + r + p + j) > 0 else 0
            )
            tpc_assist = (
                (0.0 + p + r) * 100.0 / (0.0 + f + r + p + j)
                if (f + r + p + j) > 0
                else 0
            )
            ...
            nExpulsions = (
                alumne.expulsio_set.exclude(estat="ES")
                .exclude(es_expulsio_per_acumulacio_incidencies=True)
                .count()
            )
            nExpulsionsAcu = (
                alumne.expulsio_set.exclude(estat="ES")
                .filter(es_expulsio_per_acumulacio_incidencies=True)
                .count()
            )
            camp = tools.classebuida()
            camp.enllac = (
                "/tutoria/detallTutoriaAlumne/{0}/incidencies".format(alumne.pk)
                if (nExpulsions + nExpulsionsAcu + nIncidencies + nIncidenciesInform)
                > 0
                else ""
            )
            ...
            camp = tools.classebuida()
            camp.enllac = (
                "/tutoria/detallTutoriaAlumne/{0}/actuacions".format(alumne.pk)
                if nActuacions > 0
                else ""
            )
            camp.contingut = "Act:{0}".format(nActuacions)
            filera.append(camp)
            ...
            camp = tools.classebuida()
            camp.enllac = (
                "/tutoria/detallTutoriaAlumne/{0}/families".format(alumne.pk)
                if nConnexions > 0
                else ""
            )
            camp.contingut = "Actiu:{0} Connexions:{1}".format(
                "Sí" if alumne.esta_relacio_familia_actiu() else "No", nConnexions
            )
            filera.append(camp)
            ...
            if anys_seguiment_tutorial:
                ...
                camp.contingut = "{0}".format(
                    ", ".join([unicode(x) for x in anys_seguiment_tutorial])
                )
            filera.append(camp)
            ...
            if anys_historic:
                ...
                camp.contingut = "{0}".format(
                    ", ".join([unicode(x) for x in anys_historic])
                )
            filera.append(camp)
            ...
            camp.contingut = ",".join(list(set(llistaDates)))
            filera.append(camp)
